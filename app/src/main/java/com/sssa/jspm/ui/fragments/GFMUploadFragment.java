...
import com.sssa.jspm.data.model.FileviewModel;
import com.sssa.jspm.misc.utils.Constants;
import com.sssa.jspm.misc.utils.Helper;
import com.sssa.jspm.misc.widgets.GridSpacingItemDecoration;
import com.sssa.jspm.ui.adapters.FileViewAdapter;
...
public class GFMUploadFragment extends BaseFragment {
     ...
     * @return
     */
    public static GFMUploadFragment getInstance(boolean torf, boolean toolbarconfig){
        setTorf(torf);
        setToolbarconfig(toolbarconfig);
        return new GFMUploadFragment();
    }
    ...
    private void imageviewTransition(Fragment fragment, ImageView imageView){
        ...
        Helper.setFragmentTransition(getActivity(), GFMUploadFragment.this, fragment, new Pair<View, String>(imageView,"fileviewArtwork"));
    }
    ...
    private void loadData (){
        AndroidNetworking.get(Constants.GFMFILEVIEWS)
    ...
    private void uploadData (File file, String path){
        ...
        mDialog.show();
        AndroidNetworking.upload(Constants.UPLOADGFMFILESG)
                    ...
                    public void onResponse(JSONObject response) {
                        if (response.length() > 0) {
                            try {
                                ...
                                if (!error) {
                                    Log.d("GFMUploadFragment", response.toString());
                                    String name = response.optString("name").trim();
                                    Toast.makeText(getContext(), "Uploaded "+name, Toast.LENGTH_LONG).show();
                                    fileViewAdapter.notifyDataSetChanged();
                                    mDialog.dismiss();
                                } else {
    ...
    private void filepickerDialog(){
        ...
        filePicker.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Helper.filePicker(ReqCode, GFMUploadFragment.this);
                builder.autoDismiss(true);
            }
        });
    }
